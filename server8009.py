    ...
    if CACHE is None:
        ...
        for item in raw_data:
            ...
            if siret not in associations:
                associations[siret] = {'name': item.get('name', 'Unknown'), 'siret': siret, 'mission': item.get('mission', ''), 'sectors': item.get('sectors', []), 'subventions': [], 'totalAmount': 0}
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/associations':
            ...
            filtered = [a for a in data if not search or search in a['name'].lower() or search in a['siret']]
            total = len(filtered)
            start = (page-1)*per_page
            self.send_response(200); self.send_header('Content-Type', 'application/json'); self.end_headers()
            self.wfile.write(json.dumps({'associations': filtered[start:start+per_page], 'pagination': {'page': page, 'per_page': per_page, 'total': total, 'total_pages': (total+per_page-1)//per_page}}).encode())
        elif path == '/api/stats':
            data = load_data()
            total = len(data); subv = sum(len(a['subventions']) for a in data); amt = sum(a['totalAmount'] for a in data)
            self.send_response(200); self.send_header('Content-Type', 'application/json'); self.end_headers()
            self.wfile.write(json.dumps({'totalAssociations': total, 'totalSubventions': subv, 'totalAmount': amt, 'yearRange': '-'}).encode())
